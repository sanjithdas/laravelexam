            ...
            <table  class=" table table-bordered table-striped table-hover datatable datatable-Question">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('titles.result.fields.mark_scored') }}
                        </th>
                        <th>
                            {{ trans('titles.result.fields.incorrect') }}
                        </th>
                        {{-- <th>
                            {{ trans('titles.question.fields.question_text') }}
                        </th> --}}
                        <th>
                            {{ trans('titles.result.fields.not_attended') }}
                        </th>
                        <th>
                            {{ trans('titles.result.fields.total_qns') }}
                        </th>
                        <th>
                            {{ trans('titles.result.fields.percentage') }}
                        </th>
                        <th>
                            {{ trans('titles.result.fields.updated_at') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    ...
                    @foreach($results as $key => $result)
                        <tr data-entry-id="{{ $result->userid }}">
                            <td>
                            ...
                            <td class="text text-success">
                               <b> {{ $result->success_count ?? '0'  }}</b>
                            </td>
                            <td>
                               <a class="text-danger" href="{{route('admin.results.incorrect',['user'=>$result->userid,'catid'=>$result->catid])}}"  ><b>{{ $result->failure_count ?? '0' }}</b> </a>
                            </td>
                            <td>
                                <a  class="b text-warning" href="{{route('admin.results.unanswered',['user'=>$result->userid,'catid'=>$result->catid])}}"> <b> {{ $result->unattended_count ?? '0' }} </b> </a>
                            </td>
                            <td class=" text-info">
                                <b> {{   $result->total_count ?? '0'}} </b>
                            </td>
                            <td>
                                {{ floor(($result->success_count/$result->total_count)*100) ?? '0' }}
                                {{ floor(($result->success_count/$result->total_count)*100)  > 50 ?
                                         (floor(($result->success_count/$result->total_count)*100)  > 75) ? '-Distinction':'-Passed' : '-Failed'}}
                            </td>
